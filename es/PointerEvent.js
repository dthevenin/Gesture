...
if (typeof document != "undefined" && 'createTouch' in document) EVENT_SUPPORT_TOUCH = true;else if (hasPointer || hasMSPointer) {
  EVENT_SUPPORT_TOUCH = true;
} else if (typeof document != "undefined" && window.navigator && window.navigator.userAgent) {
  if (window.navigator.userAgent.indexOf('iPhone') !== -1 || window.navigator.userAgent.indexOf('iPad') !== -1 || window.navigator.userAgent.indexOf('Android') !== -1 || window.navigator.userAgent.indexOf('BlackBerry') !== -1) {
    EVENT_SUPPORT_TOUCH = true;
  }
}
...
class Pointer {
  ...
  configureWithEvent(evt, clientX, clientY, event_bis) {
    ...
    if (evt.target) this.target = evt.target;else if (event_bis) this.target = event_bis.target;

    if (evt.currentTarget) this.currentTarget = evt.currentTarget;else if (event_bis) this.currentTarget = event_bis.currentTarget;
  }
}
...
function buildTouchList(evt, target_id) {
  ...
  for (var i = 0; i < evt.nbPointers; i++) {
    ...
    var pointer = new Pointer(touch, PointerTypes.TOUCH, touch.identifier, touch.clientX, touch.clientY, evt);
  ...
  for (var i = 0; i < evt.changedTouches.length; i++) {
    ...
    var pointer = new Pointer(touch, PointerTypes.TOUCH, touch.identifier, evt.clientX, evt.clientY, evt);
    pointers.push(pointer);
  }
...
function buildMouseList(evt, remove) {
  var pointers = [];
  pointers.push(new Pointer(evt, PointerTypes.MOUSE, MOUSE_ID, evt.layerX, evt.layerY));
  if (!remove) {
    evt.nbPointers = 1;
    evt.pointerList = pointers;
    evt.targetPointerList = pointers;
...
function getBindingIndex(target, type, listener) {
  ...
  for (var i = 0; i < listener.__event_listeners.length; i++) {
    ...
    if (binding.target === target && binding.type === type && binding.listener === listener) return i;
  }
  return -1;
}
...
/********************************************************************
                      Export
*********************************************************************/
export { createCustomEvent, removePointerListener, addPointerListener, PointerTypes, Pointer,
