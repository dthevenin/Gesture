...
function getDistance(pointer1, pointer2) {
  ...
  var x = pointer2.pageX - pointer1.pageX,
      y = pointer2.pageY - pointer1.pageY;
...
function getCentroid(pointers) {
  var nb_pointer = pointers.length,
      index = 0,
      x = 0,
      y = 0;
...
var buildPaylaod = function (event, end) {
  ...
  return {
    scale: end ? undefined : getDistance(event.targetPointerList[0], event.targetPointerList[1]) / __init_distance,
    rotation: end ? undefined : getAngle(event.targetPointerList[0], event.targetPointerList[1]) - __init_angle,
...
var gestureStartListener = function (event, listener) {
  ...
  if (!_gesture_follow) {
    __init_distance = getDistance(event.targetPointerList[0], event.targetPointerList[1]);
    __init_angle = getAngle(event.targetPointerList[0], event.targetPointerList[1]);
...
function buildGestureList(evt) {
  ...
  evt.pointerList = [new Pointer(evt, PointerTypes.TOUCH, MOUSE_ID)];
...
function touchToGestureListenerAdd(node, type, func, binding) {
  ...
  switch (type) {
    case GESTURE_START:
      binding.gesture_handler = function (e) {
        pointerStartHandler(e, gestureStartListener, target_id);
      };
  ...
  switch (type) {
    case GESTURE_START:
      binding.handler = function (e) {
        gestureIOSStartListener(e, func, target_id);
      };
      return true;
      break;
    ...
    case GESTURE_CHANGE:
      binding.handler = function (e) {
        gestureIOSChangeListener(e, func, target_id);
      };
      return true;
      break;
    ...
    case GESTURE_END:
      binding.handler = function (e) {
        gestureIOSEndListener(e, func, target_id);
      };
      return true;
      break;
  }
...
const manageGestureListenerAdd = support.gestures || support.msGestures ? gestureEventListenerAdd : touchToGestureListenerAdd;
...
const manageGestureListenerRemove = support.gestures || support.msGestures ? gestureListenerRemove : touchToGestureListenerRemove;
