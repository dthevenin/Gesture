var vs_gesture = (function (exports,vs_utils) {
'use strict';
...
var gestureStartListener = function (event, listener)
{
  ...
  if (!_gesture_follow)
  {
    ...
    __init_pos = vs_utils.getElementAbsolutePosition (event.targetPointerList[0].target, true);
//    init_pos = init_pos.matrixTransform (comp.getParentCTM ());
    
    __init_centroid = getCentroid (event.targetPointerList);
       
    document.addEventListener (exports.POINTER_MOVE, gestureChangeListener);
    document.addEventListener (exports.POINTER_END, gestureEndListener);
    document.addEventListener (exports.POINTER_CANCEL, gestureEndListener);
    createCustomEvent (exports.GESTURE_START, event.target, buildPaylaod (event));
    _gesture_follow = true;
  }
  else
  {
    createCustomEvent (exports.GESTURE_CHANGE, event.target, buildPaylaod (event));
  }
};
...
var gestureChangeListener = function (event)
{
  ...
  pointerMoveHandler (event, function (event)
  {
    ...
    if (event.targetPointerList.length < 2) {
      document.removeEventListener (exports.POINTER_MOVE, gestureChangeListener);
      document.removeEventListener (exports.POINTER_END, gestureEndListener);
      document.removeEventListener (exports.POINTER_CANCEL, gestureEndListener);
      _gesture_follow = false;
      createCustomEvent (exports.GESTURE_END, event.target, buildPaylaod (event, true));    
    }
    else {
      createCustomEvent (exports.GESTURE_CHANGE, event.target, buildPaylaod (event));
    }
...
function touchToGestureListenerAdd (node, type, func, binding)
{
  ...
  switch (type)
  {
    case exports.GESTURE_START:
      binding.gesture_handler =
        function (e) {pointerStartHandler (e, gestureStartListener, target_id);};
      node.addEventListener (exports.POINTER_START, binding.gesture_handler);
      binding.handler = func;

      return true;
    break;

    case exports.GESTURE_CHANGE:
    case exports.GESTURE_END:
      binding.handler = func;
      return true;
    break;
  }
...
function touchToGestureListenerRemove (node, type, binding)
{
  ...
  {
    case exports.GESTURE_START:
      node.removeEventListener (exports.POINTER_START, binding.gesture_handler, target_id);
...
exports.manageGestureListenerRemove = manageGestureListenerRemove;
exports.manageGestureListenerAdd = manageGestureListenerAdd;
